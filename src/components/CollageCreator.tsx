...
import { ColorSeparationModal } from "./ColorSeparationModal";
...
export type ImageObject = {
  ...
  /** SVG content from AI Vectorize — used for Color Separation */
  svgContent?: string;
};
...
export const CollageCreator = ({
  ...
  const [aiToolsInitialImageId, setAiToolsInitialImageId] = useState<string | undefined>(undefined);
  // Color Separation Modal state
  const [colorSepOpen, setColorSepOpen] = useState(false);
  const [colorSepImageId, setColorSepImageId] = useState<string | undefined>(undefined);
  const [colorSepSvg, setColorSepSvg] = useState<string>('');
  ...
  const handleToolClick = (toolId: ToolType) => {
    ...
    // AI Tools opens its own modal
    if (toolId === "ai-tools") {
      setAiToolsInitialImageId(undefined);
      setAiToolsOpen(true);
      return;
    }
  ...
  // Handle AI enhance completion — replace original image with enhanced version
  const handleAIEnhanceComplete = useCallback((imageId: string, newBlobUrl: string, newFile: File, svgContent?: string) => {
    setImages(prevImages => {
      return prevImages.map(img => {
        ...
        // Generate new thumbnail from enhanced file
        const thumbnailUrl = newBlobUrl; // Will be regenerated by ImageManager
        ...
        return {
          ...
          // Preserve or set SVG content from vectorization
          svgContent: svgContent || img.svgContent,
        };
      });
    });
    ...
    // Reset layout if this image was in it
    if (layout.length > 0 && layout.some(item => item.id === imageId)) {
      setLayout([]);
      setMultiSheetResult(null);
      setCreditsDeductedForCurrentLayout(false);
      toast.info("Layout reset due to image enhancement. Please regenerate.");
    }
  }, [layout]);
  ...
  return (
    <div className="flex flex-col pb-24 min-h-screen">
      ...
      <div className="overflow-hidden">
        ...
        <div className="px-8 md:px-16 pt-16 pb-14">
          ...
          <div className="text-center mb-10">
            <h1 className="font-heading text-4xl md:text-5xl font-extrabold mb-3 tracking-tight" style={{ color: builderSettings?.color_text || '#111827' }}>
              Create Your Gang Sheet
            </h1>
            <p className="text-lg font-medium" style={{ color: builderSettings?.color_text ? `${builderSettings.color_text}99` : '#6b7280' }}>
      ...
      <div className="px-8 md:px-16 pt-8">
        {images.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2" style={{ color: builderSettings?.color_text ? `${builderSettings.color_text}99` : '#6b7280' }}>No images uploaded yet</h3>
            <p style={{ color: builderSettings?.color_text ? `${builderSettings.color_text}66` : '#9ca3af' }}>Upload your designs above to get started</p>
          </div>
        )}
      </div>
      ...
      <AIToolsModal
        ...
        images={images}
        initialImageId={aiToolsInitialImageId}
        onEnhanceComplete={handleAIEnhanceComplete}
        onOpenColorSep={(imgId, svg) => {
          setColorSepImageId(imgId);
          setColorSepSvg(svg);
          setColorSepOpen(true);
        }}
      />

      {/* Color Separation Modal */}
      {colorSepImageId && colorSepSvg && (
        <ColorSeparationModal
          isOpen={colorSepOpen}
          onClose={() => setColorSepOpen(false)}
          svgContent={colorSepSvg}
          imageId={colorSepImageId}
          onApply={handleAIEnhanceComplete}
        />
      )}
